...
export async function fetchMonthWorklogs(creds, accountId, year, month) {
  ...
  for (const issue of issues) {
    ...
    // When total worklogs exceed the embedded page size, fetch all via dedicated endpoint
    const wlMeta = issue.fields?.worklog;
    if (wlMeta && wlMeta.total > (wlMeta.maxResults || 20)) {
      worklogs = [];
      let startAt = 0;
      const pageSize = 50;
      while (startAt < wlMeta.total) {
        const wlData = await jiraCall(
          creds,
          `/issue/${issue.key}/worklog?startAt=${startAt}&maxResults=${pageSize}`
        );
        worklogs.push(...(wlData.worklogs || []));
        startAt += pageSize;
      }
    }
...
export async function fetchUnloggedTickets(creds, accountId, year, month) {
  assertRequired(accountId, "accountId");

  const start = `${year}-${String(month + 1).padStart(2, "0")}-01`;
  const end = `${year}-${String(month + 1).padStart(2, "0")}-${lastDay(year, month)}`;
  const jql = `assignee = "${accountId}" AND updated >= "${start}" AND updated <= "${end}" AND timespent is EMPTY ORDER BY updated DESC`;
